...
- K1-S0.5 (issue #20): user-visible product identity is `Arckeep` (productName, window title, shell brand, icons, artifact `Arckeep-*-x64.zip`). Internal identifiers intentionally stay KCC: repo/npm name, appId `com.local.kccworkbench`, `KCC_PROFILE_STARTUP`, and the stable userData directory `%APPDATA%\KCC Workbench` (explicitly pinned in `src/main/main.js`; do not rename it or state continuity breaks).
